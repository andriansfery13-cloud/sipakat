<x-app-layout>
    <x-slot name="title">Edit Kecamatan</x-slot>
    <x-slot name="subtitle">Formulir perubahan data kecamatan (Tenant).</x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-2xl animate-fade-in-up animate-delay-1">
        <div class="p-6 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
            <h3 class="text-base font-bold text-slate-800">Formulir Ubah Data Kecamatan</h3>
        </div>
        
        <form action="{{ route('superadmin.kecamatans.update', $kecamatan->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="nama_kecamatan" id="nama_kecamatan" value="{{ old('nama_kecamatan', $kecamatan->nama_kecamatan) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="kode_kecamatan" id="kode_kecamatan" value="{{ old('kode_kecamatan', $kecamatan->kode_kecamatan) }}" required
                        class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none"
                ...
                <div>
                    ...
                    <textarea name="alamat" id="alamat" rows="3"
                        class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none"
                        placeholder="Masukkan alamat lengkap kantor kecamatan (Opsional)">{{ old('alamat', $kecamatan->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-xs text-rose-500 flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            {{ $message }}
